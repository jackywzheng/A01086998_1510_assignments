...
class TestCombatRound(TestCase):
    ...
    # This tests for who strikes first and if message is correct
    ...
    def test_combat_round_character_2_remaining_HP(self, mock_stdout, mock_roll_die,
    ...
    @patch('dungeonsanddragons.opponent_one_attack', return_value=0)
    @patch('dungeonsanddragons.opponent_two_attack', return_value=1)
    @unittest.mock.patch('sys.stdout', new_callable=io.StringIO)
    def test_combat_round_character_2_dead(self, mock_stdout, mock_roll_die,
                                           mock_opponent_one_attack, mock_opponent_two_attack):
        character_1 = {'Name': 'Jaki', 'Class': 'fighter', 'HP': 9, 'Strength': 18, 'Dexterity': 18, 'Constitution': 18,
                       'Intelligence': 18, 'Wisdom': 18, 'Charisma': 18, 'XP': 0}
